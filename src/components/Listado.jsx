  ...
  const renderSolicitudesPorTipo = (tipo) => {
    return solicitudes
      ...
      .map((s) => (
        <BasicCard
          key={s._id}
          nombreSolicitud={s.tipo + ' - ' + s.paciente.nombre}
          descripcion={s.observaciones}
          fecha={s.fechaPrestacion ?? '----/--/--'}
          selected={selectedId === s._id}
          onSelect={(id) => setSelectedId(s._id)}
        ...
        <Stack direction='row' sx={{ width:'100%', flex: 1,maxHeight: '80vh', overflowY: "auto"}}>
          <TabPanel value={value} index={0} dir={theme.direction}>
            {solicitudes.map((p) => (
              <BasicCard
                key={p._id}
                nombreSolicitud={p.tipo + ' - ' + p.paciente.nombre}
                descripcion={p.observaciones}
                fecha={p.fechaPrestacion ? new Date(p.fechaPrestacion).toLocaleDateString('es-AR') : '--/--/----'}
