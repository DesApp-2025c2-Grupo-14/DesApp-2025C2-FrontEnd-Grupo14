import { Stack } from "@mui/material";
import { DatosProvider  } from "../context/datos";
import { BuscadorPacientes } from "../components/BuscadorPacientes";
import { InfoContainer } from "../components/InfoContainer";
import { Header } from '../components/Header';


export function Pacientes() {
  return (
    <Stack direction='column' width='100%' height='100%' paddingLeft={2} bgcolor='#6793e6' spacing={2}>
        <Header seccion='Pacientes' usuario='Ariel Nuñez' />
      <Stack direction="row" spacing={4} height="90%">
        <DatosProvider>
          <Stack width="50%" padding="30px" sx={{ bgcolor: "#aec3f3", borderRadius: 3, border: "2px solid"}}>
            <BuscadorPacientes />
          </Stack>
          <Stack width="100%" paddingLeft="10px" sx={{ bgcolor: "#aec3f3",   borderRadius: 3, border: "2px solid"}}>
            <InfoContainer />
          </Stack>
        </DatosProvider>
      </Stack>
    </Stack>
  );
}
